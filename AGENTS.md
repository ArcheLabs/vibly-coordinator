...
`vibly-coordinator` is the **single source of truth for the Coordinator HTTP/SSE contract**. The `@vibly-ai/coordinator-http-contract` package and all consumers (`vibly-client`, `vibly-console`) derive their request/response types from the OpenAPI document this app emits via `@fastify/swagger`.
...
3. **Adding/changing a route MUST refresh contract types.** Run `pnpm --filter @vibly-ai/coordinator-http-contract gen` and commit the new `src/generated/types.ts`. CI runs `verify:types` to detect drift.
...
5. **Do not introduce a competing HTTP path table or "second source of truth" client wrapper inside this app.** Consumers must consume `@vibly-ai/coordinator-http-contract` directly; this app does not ship a separate REST client. CLI/dev scripts that need to call the API can use the contract client themselves.
